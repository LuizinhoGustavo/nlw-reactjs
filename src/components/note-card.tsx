...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps){ // Chama a interface e criar o recurso Props
    return(
      <Dialog.Root>
          ...
          <Dialog.Portal> {/*Ao criar o element ele redireciona ele para o body da página e não segue o grid, por exemplo*/}
            <Dialog.Overlay className="inset-0 fixed bg-black/40"> {/* Overlay é o fundo do modal, que faz o efeito que o Modal está em cima do conteúdo*/}
              <Dialog.Content className="fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 overflow-hidden bg-slate-700 md:rounded-md max-w-[640px] w-full md:h-[60vh] flex flex-col"> {/*Conteúdo que vai ser gerado ao presionar o trigger*/}
                ...
                <button type="button" onClick={() => onNoteDeleted(note.id)} className="w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none font-medium group">Deseja <span className="text-red-400 group-hover:underline">apagar esse nota?</span></button>
              </Dialog.Content>
            </Dialog.Overlay>
          </Dialog.Portal>
      </Dialog.Root>
